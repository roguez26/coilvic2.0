...
public class CollaborativeProjectRequestDAO implements ICollaborativeProjectRequest {
            ...
            switch (collaborativeProjectRequest.getRequesterCourse().getStatus()) {
                case "Aceptado" -> {
                    switch (collaborativeProjectRequest.getRequestedCourse().getStatus()) {
                        ...
                        case "Colaboracion" ->
                            throw new DAOException("No se pudo enviar la solicitud, el curso que solicitó ya forma parte de un proyecto colaborativo", Status.WARNING);
                        default -> {
                        }
                    }
                }
                case "Pendiente" ->
                    throw new DAOException("No se pudo enviar la solicitud, su curso aun no ha sido aceptado", Status.WARNING);
                case "Rechazado" ->
                    throw new DAOException("No se pudo enviar la solicitud, su curso fue rechazado", Status.WARNING);
                case "Colaboracion" ->
                    throw new DAOException("No se pudo enviar la solicitud, el curso ya forma parte de un proyecto colaborativo", Status.WARNING);
                default -> {
                }
            }
        } else {
    ...
    public int cancelCollaborativeProjectRequest(CollaborativeProjectRequest collaborativeProjectRequest) throws DAOException {
        ...
        switch (collaborativeProjectRequest.getStatus()) {
            case "Pendiente" ->
                result = cancelCollaborativeProjectRequestStatusById(collaborativeProjectRequest.getIdCollaboratibeProjectRequest());
            case "Aceptado" ->
                throw new DAOException("No puede cancelar una solicitud de proyecto colaborativo que ya fue aceptada", Status.WARNING);
            case "Rechazado" ->
                throw new DAOException("No puede cancelar una solicitud de proyecto colaborativo que ya fue rechazada", Status.WARNING);
            default -> {
            }
        }
        return result;
